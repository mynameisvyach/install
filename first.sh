...
echo -e "${YELLOW}[4/10] Проверка и установка SSH сервера...${NC}"
...
echo -e "${YELLOW}[5/10] Вопрос пользователю о ботах...${NC}"
while true; do
...
echo -e "${YELLOW}[6/10] Настройка виртуального окружения Python...${NC}"
...
# Вопрос про сервисные файлы
echo -e "${YELLOW}[7/10] Вопрос пользователю о сервисных файлах...${NC}"
while true; do
    read -p "Вы закинули service-файлы для ботов в /etc/systemd/system/? (yes/no): " service_answer
    case $service_answer in
        [Yy]*|"yes"|"YES")
            echo -e "${GREEN}Отлично! Продолжаем настройку systemd...${NC}"
            break
            ;;
        [Nn]*|"no"|"NO")
            echo -e "${RED}Пожалуйста, создайте service-файлы для ботов в /etc/systemd/system/ и запустите скрипт снова.${NC}"
            echo -e "${YELLOW}Пример создания service-файла:${NC}"
            echo -e "sudo nano /etc/systemd/system/actsbot.service"
            echo -e "${YELLOW}С содержимым:${NC}"
            echo -e "[Unit]\nDescription=Acts Telegram Bot\nAfter=network.target\n\n[Service]\nType=simple\nUser=$USER\nWorkingDirectory=/home/TelegramBots\nExecStart=/home/TelegramBots/venv/bin/python3 /home/TelegramBots/actsbot.py\nRestart=always\nRestartSec=10\n\n[Install]\nWantedBy=multi-user.target"
            exit 1
            ;;
        *)
            echo "Пожалуйста, ответьте yes или no"
...
# Настройка и запуск systemd сервисов ботов
echo -e "${YELLOW}[8/10] Настройка и запуск systemd сервисов ботов...${NC}"
...
# Запуск каждого бота
for bot in "${BOTS[@]}"; do
    echo -e "${YELLOW}Запуск $bot...${NC}"
    if sudo systemctl start $bot 2>/dev/null; then
        echo -e "${GREEN}$bot успешно запущен${NC}"
    else
        echo -e "${RED}Ошибка при запуске $bot. Возможно, service-файл не существует или имеет ошибки.${NC}"
    fi
done

# Проверка статуса ботов
echo -e "${YELLOW}Проверка статуса ботов:${NC}"
for bot in "${BOTS[@]}"; do
    if systemctl list-unit-files | grep -q "$bot.service"; then
        echo -e "${YELLOW}--- Статус $bot ---${NC}"
        sudo systemctl status $bot --no-pager -l
    else
        echo -e "${RED}--- $bot: service-файл не найден ---${NC}"
    fi
    echo ""
done

# Включение автозапуска ботов
echo -e "${YELLOW}Включение автозапуска ботов...${NC}"
for bot in "${BOTS[@]}"; do
    if sudo systemctl enable $bot 2>/dev/null; then
        echo -e "${GREEN}$bot добавлен в автозапуск${NC}"
    else
        echo -e "${RED}Ошибка при добавлении $bot в автозапуск (возможно, service-файл не существует)${NC}"
    fi
done

# Настройка SSH (PubkeyAuthentication)
echo -e "${YELLOW}[9/10] Настройка SSH (PubkeyAuthentication)...${NC}"
...
for bot in "${BOTS[@]}"; do
    if systemctl is-active --quiet $bot 2>/dev/null; then
        echo -e "${GREEN}✓ $bot - активен${NC}"
    elif systemctl list-unit-files | grep -q "$bot.service"; then
        echo -e "${RED}✗ $bot - не активен (проверьте лог: journalctl -u $bot)${NC}"
    else
        echo -e "${RED}✗ $bot - service-файл не найден${NC}"
    fi
done
